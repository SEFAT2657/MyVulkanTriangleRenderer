...
////////////iterate voer presentation modes and select current mode with desired mode used to qualify
...
/////After here swapchain creation
    ...
    std::vector<VkImage> swapchain_images(4);
...
/////Swapchain creation succcccesss
    ...
    if(vkGetSwapchainImagesKHR(logical_device,swapchain,&swapchain_images_count,nullptr)!=VK_SUCCESS)
    {
        std::cout<<"There was an error with Getting SwapChain Images\n";
        return false;
    }
    ...
    for(size_t i=0;i<swapchain_images_count;i++){
        ...
        createInfo.format = image_format; // Format of the swapchain images
    ...
    std::vector<VkFramebuffer> frameBuffers(swapChainImageViews.size());
        ...
        void* data;
        vkMapMemory(logical_device,vertexBufferMemory,0,bufferinfo.size,0,&data);
        ...
        colorBlendAttachment.blendEnable = VK_TRUE;  // No blending, just overwrite framebuffer
        ...
        viewport.x=0.0f;
        viewport.y=0.0f;
        viewport.width=(float)size_of_images.width;
        viewport.height=(float)size_of_images.height;
    ...
    //scisssor
        ...
        renderBeginPassInfo.framebuffer=frameBuffers[0];
        renderBeginPassInfo.renderArea.offset={0,0};
        renderBeginPassInfo.renderArea.extent=size_of_images;
        ...
        if(presentResult!=VK_SUCCESS){
            std::cout<<"presentResult isnt Successfull"<<std::endl;
        }
